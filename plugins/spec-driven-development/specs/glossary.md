...
**Test Builder**
A Builder Agent operating in Wave 1 of the two-wave execution model. Writes test files, type stubs, and fixtures only. Must run the quality gate and confirm the red phase (all tests fail) before reporting success.
*Cannot:* write production implementation code; report success while any test passes; skip the quality gate or spec coverage check.

**Code Builder**
A Builder Agent operating in Wave 2 of the two-wave execution model. Implements production code to make Wave 1 tests pass, then runs the spec-implemented gate to verify full behavioral spec coverage.
*Cannot:* write test files (Wave 1's responsibility); declare completion while any Wave 1 test fails or any spec requirement is uncovered.
...
A markdown file in specs/ produced by /defining-test-scenarios, named `{spec-name}.test.{FXX}.md`. Contains exhaustive GIVEN-WHEN-THEN test scenarios for one feature. Used by Test Builders in Wave 1 to verify coverage, and referenced by planners so they can include the file path in their output.
