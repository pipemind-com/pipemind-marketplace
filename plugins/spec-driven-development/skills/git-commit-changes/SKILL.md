...
- NEVER include `Co-Authored-By` lines in commit messages — specifically, never write `Co-Authored-By: Claude Opus X.X <noreply@anthropic.com>` or any variant
- NEVER amend existing commits
- NEVER push to remote
- Each commit must be a single logical unit: one feature (code and test), one fix, one refactor, one chore
